...
### 2. Check GitHub issues (if GitHub MCP is available)

- Call `get_me` to get repo context, then `search_issues` (or `list_issues`) for the current repository with state `open`.
...
- **Issue references**: If a commit fixes or implements a GitHub issue, add `Fixes #N` or `Closes #N` in the body (or ` (fixes #N)` in the subject). GitHub will auto-close and link the issue.
...
# If the fix(decompose) commit resolves GitHub issue #12:
# git add decompose/index.ts && git commit -m "fix(decompose): correct section index and guard empty headings" -m "Fixes #12" --trailer "Co-authored-by: ..."
```
