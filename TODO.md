...
- [ ] **Flatten git tree** — Flatten submodules into the monorepo with `git subtree add` (deinit/remove each submodule first, then subtree add per repo) so full history lives in the monorepo. Optionally use `.gittrees` + push script to push subtrees back to separate remotes.
