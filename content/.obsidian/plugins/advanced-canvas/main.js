...
var DebugHelper = class {
  constructor(plugin) {
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:canvas-changed", (_canvas) => {
        this.nodeAddedCount = 0
        this.nodeChangedCount = 0
        this.edgeAddedCount = 0
        this.edgeChangedCount = 0
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-added", (_canvas, _node) => {
        if (this.logging) console.count("\u{1F7E2} NodeAdded")
        this.nodeAddedCount++
      }),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-changed", (_canvas, _node) => {
        if (this.logging) console.count("\u{1F7E1} NodeChanged")
        this.nodeChangedCount++
      }),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:edge-added", (_canvas, _edge) => {
        if (this.logging) console.count("\u{1F7E2} EdgeAdded")
        this.edgeAddedCount++
      }),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:edge-changed", (_canvas, _edge) => {
...
var _CanvasHelper = class _CanvasHelper {
  ...
  static addCardMenuOption(canvas, element) {
    var _a
    if (element.id)
      (_a = canvas == null ? void 0 : canvas.cardMenuEl.querySelector(`#${element.id}`)) == null
        ? void 0
        : _a.remove()
    ...
    const bBoxes = canvasElements
      .map((element) => {
        ...
        if (
          nodeData.x !== void 0 &&
          nodeData.y !== void 0 &&
          nodeData.width !== void 0 &&
          nodeData.height !== void 0
        )
          return {
            minX: nodeData.x,
            minY: nodeData.y,
    ...
    for (const stylableAttribute of stylableAttributes) {
      const selectedStyle =
        (_a = stylableAttribute.options.find(
          (option) => currentStyleAttributes[stylableAttribute.key] === option.value,
        )) != null
          ? _a
  ...
  static addStyleAttributesDropdownMenu(
    ...
    styleMenuButtonElement.addEventListener("click", () => {
      ...
      if (!isOpen) {
        ;(_a2 = popupMenuElement.querySelector(`#${STYLE_MENU_DROPDOWN_ID}`)) == null
          ? void 0
          : _a2.remove()
        ;(_b2 = popupMenuElement.querySelector(`#${STYLE_MENU_DROPDOWN_SUBMENU_ID}`)) == null
          ? void 0
          : _b2.remove()
        return
      }
      ...
      for (const stylableAttribute of stylableAttributes) {
        ...
        stylableAttributeElement.addEventListener("click", () => {
          var _a3
          ;(_a3 = popupMenuElement.querySelector(`#${STYLE_MENU_DROPDOWN_SUBMENU_ID}`)) == null
            ? void 0
            : _a3.remove()
          ...
          const topOffset =
            parseFloat(
              window.getComputedStyle(styleMenuDropdownElement).getPropertyValue("padding-top"),
            ) +
            (styleMenuDropdownElement.offsetHeight - styleMenuDropdownElement.clientHeight) / 2
...
var EdgePathfindingAStar = class extends EdgePathfindingMethod {
  getPath() {
    ...
    if (roundPath) {
      ...
      else svgPath = SvgPathHelper.pathArrayToRoundedSvgPath(pathArray, ROUND_PATH_RADIUS)
  ...
  aStarAlgorithm(fromPos, toPos, obstacles, gridResolution, allowDiagonal) {
    ...
    while (openSet.length > 0) {
      ...
      if (current.x === end.x && current.y === end.y)
        return [fromPos, ...this.reconstructPath(current), toPos].map((node) => ({
          x: node.x,
          y: node.y,
        }))
      if (
...
var EdgePathfindingSquare = class extends EdgePathfindingMethod {
  ...
  getZPath(fromPos, toPos, fromSide, toSide) {
    if (BBoxHelper.isHorizontal(fromSide)) {
      ...
      return {
        pathArray: [fromPos, { x: midX, y: fromPos.y }, { x: midX, y: toPos.y }, toPos],
        center: {
          x: midX,
          y: (fromPos.y + toPos.y) / 2,
        },
      ...
      return {
        pathArray: [fromPos, { x: fromPos.x, y: midY }, { x: toPos.x, y: midY }, toPos],
        center: {
          x: (fromPos.x + toPos.x) / 2,
          y: midY,
        },
...
var CssStylesConfigManager = class {
  ...
  parseStyleConfigsFromCSS(sheet) {
    var _a, _b
    const textContent =
      (_b = (_a = sheet == null ? void 0 : sheet.ownerNode) == null ? void 0 : _a.textContent) ==
      null
        ? void 0
        : _b.trim()
...
var EdgeStylesExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:popup-menu-created", (canvas) =>
        this.onPopupMenuCreated(canvas),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:edge-changed", (canvas, edge) =>
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-added", (canvas, node) => {
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
        ...
        (canvas, node, keyboard) =>
          node.initialized && keyboard ? this.updateAllEdgesInArea(canvas, node.getBBox()) : void 0,
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-removed", (canvas, node) =>
        this.updateAllEdgesInArea(canvas, node.getBBox()),
      ),
    )
    this.plugin.registerEvent(
  ...
  getArrowPolygonPoints(arrowStyle) {
    ...
    else if (arrowStyle === "thin-triangle") return `0,0 7,10 0,0 0,10 0,0 -7,10`
...
var NodeStylesExtension = class extends CanvasExtension {
  ...
  onPopupMenuCreated(canvas) {
    ...
    const availableNodeStyles = [
      ...
      (style) => !style.nodeTypes || style.nodeTypes.some((type) => selectedNodeTypes.has(type)),
    )
    CanvasHelper.addStyleAttributesToPopup(
      this.plugin,
      canvas,
...
var AdvancedCanvasPluginSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  createTextSetting(containerEl, settingId, setting) {
    new import_obsidian4.Setting(containerEl)
      ...
      .addText((text) =>
        text.setValue(this.settingsManager.getSetting(settingId)).onChange(async (value) => {
  ...
  createDimensionSetting(containerEl, settingId, setting) {
    ...
    new import_obsidian4.Setting(containerEl)
      ...
      .setDesc(setting.description)
      .addText((text) => {
        text1 = text.setValue(this.settingsManager.getSetting(settingId)[0].toString()).onChange(
          async (value) =>
            await this.settingsManager.setSetting({
      ...
      .addText((text) => {
        text2 = text.setValue(this.settingsManager.getSetting(settingId)[1].toString()).onChange(
          async (value) =>
            await this.settingsManager.setSetting({
  ...
  createBooleanSetting(containerEl, settingId, setting) {
    new import_obsidian4.Setting(containerEl)
      ...
      .setDesc(setting.description)
      .addToggle((toggle) =>
        toggle.setValue(this.settingsManager.getSetting(settingId)).onChange(async (value) => {
  ...
  createButtonSetting(containerEl, settingId, setting) {
    new import_obsidian4.Setting(containerEl)
      .setName(setting.label)
      .setDesc(setting.description)
      .addButton((button) => button.setButtonText("Open").onClick(() => setting.onClick()))
  }
      ...
      new import_obsidian4.Setting(nestedContainerEl)
        ...
        .addDropdown((dropdown) => {
          ...
          return dropdown
            ...
            .setValue(
              (_a = this.settingsManager.getSetting(settingId)[styleAttribute.key]) != null
                ? _a
                : "null",
            )
            .onChange(async (value) => {
...
var WindowsManager = class {
  constructor(plugin) {
    ...
    this.plugin = plugin
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("window-open", (_win, window2) => this.windows.push(window2)),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
        "window-close",
...
var _MigrationHelper = class _MigrationHelper {
  ...
  static migrate(canvas) {
    var _a, _b
    let version =
      (_b = (_a = canvas.metadata) == null ? void 0 : _a.version) != null ? _b : "undefined"
...
var CanvasPatcher = class extends Patcher {
  async patch() {
    const loadedCanvasViewLeafs = this.plugin.app.workspace
      ...
      .filter((leaf) => !(0, import_obsidian5.requireApiVersion)("1.7.2") || !leaf.isDeferred)
      ...
      setEphemeralState: Patcher.OverrideExisting(
        (next) =>
          function (state) {
            ...
            if (
              ((_b = (_a = state.match) == null ? void 0 : _a.matches) == null ? void 0 : _b[0]) &&
...
var LinkSuggestionsPatcher = class extends Patcher {
  async patch() {
    ...
    const suggestManager =
      (_a = this.plugin.app.workspace.editorSuggest.suggests.find((s) => s.suggestManager)) == null
        ? void 0
        : _a.suggestManager
  ...
  static async computeCanvasMetadataAsync(data) {
    ...
    for (const [key, value] of Object.entries(frontmatter != null ? frontmatter : {})) {
      ...
      if (typeof value === "string")
        (_b = metadata.frontmatterLinks) == null ? void 0 : _b.push(...getLinks([value]))
      else if (Array.isArray(value))
        (_c = metadata.frontmatterLinks) == null ? void 0 : _c.push(...getLinks(value))
    }
    metadata.nodes = {}
    metadata.links = []
    metadata.embeds = []
            ...
            function (file, content) {
              ...
              if ((file == null ? void 0 : file.extension) === "canvas") {
                ...
                try {
                  frontmatter =
                    (_c =
                      (_b = (_a = JSON.parse(content)) == null ? void 0 : _a.metadata) == null
                        ? void 0
                        : _b.frontmatter) != null
                      ? _c
                      : {}
      ...
      _match: Patcher.OverrideExisting(
        (next) =>
          function (data) {
            var _a, _b
            const isCanvas =
              (_b = (_a = data.strings.filepath) == null ? void 0 : _a.endsWith(".canvas")) != null
                ? _b
                : false
...
var MetadataCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:canvas-metadata-changed", (canvas) =>
        this.onMetadataChanged(canvas),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:canvas-view-unloaded:before", (view) =>
        this.onCanvasViewUnloaded(view),
      ),
    )
  }
  onCanvasChanged(canvas) {
    ...
    if (!metadata || metadata.version !== CURRENT_SPEC_VERSION)
      return new import_obsidian11.Notice(
...
var NodeRatioCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("canvas:node-menu", (menu, node) => this.onNodeMenu(menu, node)),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-resized", (canvas, node) =>
        this.onNodeResized(canvas, node),
      ),
    )
  }
  ...
  onNodeResized(_canvas, node) {
    ...
    if (nodeAspectRatio < nodeData.ratio) nodeSize.width = nodeSize.height * nodeData.ratio
...
var PresentationCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.addCommand({
      ...
      checkCallback: CanvasHelper.canvasCommand(
        ...
        (canvas) =>
          this.setStartNode(canvas, canvas.nodes.get(canvas.getSelectionData().nodes[0].id)),
      ),
    })
    this.plugin.addCommand({
  ...
  async startPresentation(canvas, tryContinue = false) {
    if (!tryContinue || this.visitedNodeIds.length === 0) {
      const startNode2 =
        canvas.metadata["startNode"] && canvas.nodes.get(canvas.metadata["startNode"])
      if (!startNode2) {
        new import_obsidian13.Notice(
          "No start node found. Please mark a node as a start node trough the popup menu.",
        )
        return
      }
  ...
  nextNode(canvas) {
    ...
    if (outgoingEdges.length > 1) {
      ...
      const traversedEdgesCount =
        this.visitedNodeIds.filter((visitedNodeId) => visitedNodeId === fromNodeId).length - 1
...
var ZOrderingCanvasExtension = class extends CanvasExtension {
  ...
  addZOrderingContextMenuItems(canvas, nodes, menu) {
    menu.addSeparator()
    if (
      this.plugin.settings.getSetting("zOrderingControlShowOneLayerShiftOptions") &&
      nodes.length === 1
    ) {
      menu.addItem((item) => {
  ...
  moveOneLayer(canvas, selectedNode, forward) {
    ...
    const collidingNodes = [...canvas.nodes.values()]
      .filter((node) => BBoxHelper.isColliding(selectedNodeBBox, node.getBBox()))
      .filter((node) => node !== selectedNode)
    const nearestZIndexNode = collidingNodes
      .sort((a, b) => (forward ? a.zIndex - b.zIndex : b.zIndex - a.zIndex))
      .filter((node) =>
        forward ? node.zIndex > selectedNode.zIndex : node.zIndex < selectedNode.zIndex,
      )
      .first()
...
var BetterReadonlyCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:viewport-changed:before", (canvas) =>
        this.onBeforeViewPortChanged(canvas),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:readonly-changed", (canvas, _readonly) => {
        this.updatePopupMenu(canvas)
...
var EncapsulateCanvasExtension = class extends CanvasExtension {
  ...
  async encapsulateSelection(canvas) {
    ...
    if (defaultNewCanvasLocation === "current")
      targetFolderPath =
        (_c =
          (_b = (_a = canvas.view.file) == null ? void 0 : _a.parent) == null ? void 0 : _b.path) !=
        null
          ? _c
          : targetFolderPath
...
var CommandsCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.addCommand({
      ...
      checkCallback: CanvasHelper.canvasCommand(
        ...
        (_canvas) => true,
        (canvas) =>
          canvas.updateSelection(() => (canvas.selection = new Set(canvas.edges.values()))),
      ),
    })
    this.plugin.addCommand({
        ...
        (canvas) => {
          ...
          for (const nodeId of selectedNodeIds) {
            ...
            let nodeOutgoingLinks =
              (_b = (_a = metadata.nodes) == null ? void 0 : _a[nodeId]) == null ? void 0 : _b.links
            ...
            for (const nodeOutgoingLink of nodeOutgoingLinks) {
              const resolvedLink = this.plugin.app.metadataCache.getFirstLinkpathDest(
  ...
  async createFileNode(canvas, file) {
    ...
    file != null
      ? file
      : (file = await new FileSelectModal(this.plugin.app, void 0, true).awaitInput())
  ...
  cloneNode(canvas, cloneDirection) {
    ...
    if (this.plugin.settings.getSetting("zoomToClonedNode")) canvas.zoomToBbox(clonedNode.getBBox())
  }
  ...
  getNextNode(canvas, direction) {
    ...
    const closestNode = possibleTargetNodes.reduce((closestNode2, node) => {
      ...
      const isInVerticalRange =
        selectedNodeBBox.minY <= nodeBBox.maxY && selectedNodeBBox.maxY >= nodeBBox.minY
      const isInHorizontalRange =
        selectedNodeBBox.minX <= nodeBBox.maxX && selectedNodeBBox.maxX >= nodeBBox.minX
...
var AutoResizeNodeCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-created", (canvas, node) =>
        this.onNodeCreated(canvas, node),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:popup-menu-created", (canvas) =>
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
        ...
        (canvas, node, viewUpdate) =>
          this.onNodeTextContentChanged(canvas, node, viewUpdate.view.dom),
      ),
    )
  }
  ...
  onPopupMenuCreated(canvas) {
    ...
    CanvasHelper.addPopupMenuOption(
      ...
      CanvasHelper.createPopupMenuOption({
        ...
        callback: () =>
          this.toggleAutoResizeHeightEnabled(canvas, selectedNodes, autoResizeHeightEnabled),
      }),
    )
  }
...
var PortalsCanvasExtension = class _PortalsCanvasExtension extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.vault.on("modify", (file) => {
        for (const canvas of this.plugin.getCanvases()) this.onFileModified(canvas, file)
      }),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:popup-menu-created", (canvas) =>
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-removed", (canvas, node) =>
        this.onNodeRemoved(canvas, node),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-moved", (canvas, node, _keyboard) =>
        this.onNodeMoved(canvas, node),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-resized", (canvas, node) =>
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
        ...
        (canvas, edge, event, cancelRef) =>
          this.onEdgeConnectionTryDraggingBefore(canvas, edge, event, cancelRef),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
        ...
        (canvas, edge, event, newEdge, side, previousEnds) =>
          this.onEdgeConnectionDraggingAfter(canvas, edge, event, newEdge, side, previousEnds),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:data-requested", (canvas, data) =>
        this.onGetData(canvas, data),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
  ...
  onFileModified(canvas, file) {
    const isAffected =
      Object.values(canvas.nodes).filter(
        (nodeData) =>
          nodeData.getData().type === "file" && nodeData.currentPortalFile === file.path,
      ).length > 0
  ...
  onSelectionChanged(canvas, _oldSelection, updateSelection) {
    updateSelection(() => {
      const updatedSelection = Array.from(canvas.selection).filter(
        (canvasElement) => !_PortalsCanvasExtension.isPortalElement(canvasElement.id),
      )
  ...
  onNodeRemoved(canvas, portalNode) {
    ...
    for (const node of this.getContainingNodes(canvas, portalNode, false)) canvas.removeNode(node)
    for (const edge of this.getContainingEdges(canvas, portalNode, false)) canvas.removeEdge(edge)
  }
  ...
  async tryOpenPortal(canvas, portalNodeData, nestedPortalFiles = /* @__PURE__ */ new Set()) {
    ...
    if (!(portalFile instanceof import_obsidian15.TFile) || portalFile.extension !== "canvas")
      return addedData
    ...
    for (const nodeDataFromPortal of portalFileData.nodes) {
      ...
      if (!newNodeId.startsWith(PORTAL_ID_PREFIX)) newNodeId = PORTAL_ID_PREFIX + newNodeId
      const addedNode = {
        ...nodeDataFromPortal,
        id: newNodeId,
    ...
    for (const edgeDataFromPortal of portalFileData.edges) {
      ...
      if (!newEdgeId.startsWith(PORTAL_ID_PREFIX)) newEdgeId = PORTAL_ID_PREFIX + newEdgeId
  ...
  getContainingNodes(canvas, portalNode, directChildren = true) {
    return Array.from(canvas.nodes.values()).filter((node) =>
      this.isChildOfPortal(portalNode.getData(), node.getData(), directChildren),
    )
  }
  getContainingEdges(canvas, portalNode, directChildren = true) {
    return Array.from(canvas.edges.values()).filter((edge) =>
      this.isChildOfPortal(portalNode.getData(), edge.getData(), directChildren),
    )
  }
    ...
    CanvasHelper.addControlMenuButton(
      ...
      CanvasHelper.createControlMenuButton({
        ...
        callback: () => {
          ...
          let propertiesLeaf =
            (_a2 = this.plugin.app.workspace.getLeavesOfType("file-properties").first()) != null
              ? _a2
              : null
          if (!propertiesLeaf) {
...
var BetterDefaultSettingsCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:edge-created", (canvas, edge) =>
        this.applyDefaultEdgeStyles(canvas, edge),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-resized", (canvas, node) =>
        this.enforceMaxNodeWidth(canvas, node),
      ),
    )
  }
...
var ColorPaletteCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("css-change", () => this.updateCustomColorModStyleClasses()),
    )
    this.updateCustomColorModStyleClasses()
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:popup-menu-created", (canvas) =>
        this.patchColorSelection(canvas),
      ),
    )
...
var CollapsibleGroupsCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-changed", (canvas, node) =>
        this.onNodeChanged(canvas, node),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-bbox-requested", (canvas, node, bbox) =>
        this.onNodeBBoxRequested(canvas, node, bbox),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:copy", (canvas, selectionData) =>
        this.onCopy(canvas, selectionData),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:data-requested", (_canvas, data) =>
      ...
      : _a.forEach((groupNodeData) => {
          ...
          const containedNodesData = data.nodes.filter(
            (nodeData) =>
              ...
              BBoxHelper.insideBBox(CanvasHelper.getBBox([nodeData]), groupNodeBBox, false),
          )
          const containedEdgesData = data.edges.filter((edgeData) => {
            return (
              containedNodesData.some((nodeData) => nodeData.id === edgeData.fromNode) ||
          ...
          const newContainedNodesData = containedNodesData.filter((nodeData) => {
            var _a3, _b2, _c2
            return !((_c2 =
              (_b2 = (_a3 = groupNodeData.collapsedData) == null ? void 0 : _a3.nodes) == null
                ? void 0
          ...
          const newContainedEdgesData = containedEdgesData.filter((edgeData) => {
            var _a3, _b2, _c2
            return !((_c2 =
              (_b2 = (_a3 = groupNodeData.collapsedData) == null ? void 0 : _a3.edges) == null
                ? void 0
          ...
          groupNodeData.collapsedData = {
            nodes: [
              ...((_b = (_a2 = groupNodeData.collapsedData) == null ? void 0 : _a2.nodes) != null
                ? _b
                : []),
            ...
            edges: [
              ...((_d = (_c = groupNodeData.collapsedData) == null ? void 0 : _c.edges) != null
                ? _d
                : []),
...
var FocusModeCanvasExtension = class extends CanvasExtension {
  ...
  toggleFocusMode(canvas) {
    ...
    const controlMenuFocusToggle =
      (_b = (_a = canvas.quickSettingsButton) == null ? void 0 : _a.parentElement) == null
        ? void 0
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.metadataCache.on("changed", (file) => {
        for (const canvas of this.plugin.getCanvases()) this.onMetadataChanged(canvas, file)
      }),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-added", (canvas, node) =>
  ...
  getFileNodeEdges(canvas, node) {
    ...
    const fileLinksToBeLinkedTo =
      (_b =
        (_a = fileMetadata.frontmatterLinks) == null
          ? void 0
          : _a.filter((link) => link.key.split(".")[0] === linkedFilesFrontmatterKey)) != null
        ? _b
        : []
    ...
    const nodesToBeLinkedTo = Array.from(canvas.nodes.values()).filter((otherNode) => {
      ...
      return (
        ...
        filepathsToBeLinkedTo.includes((_a2 = otherNode.file) == null ? void 0 : _a2.path)
      )
    })
...
var FlipEdgeCanvasExtension = class extends CanvasExtension {
  ...
  onPopupMenuCreated(canvas) {
    ...
    for (const icon of POSSIBLE_ICONS) {
      edgeDirectionButton =
        (_b = popupMenuEl.querySelector(`button:not([id]) > .svg-icon.${icon}`)) == null
          ? void 0
          : _b.parentElement
...
var ExportCanvasExtension = class extends CanvasExtension {
  ...
  async showExportImageSettingsModal(canvas, nodesToExport) {
    ...
    const updateDynamicSettings = () => {
      var _a, _b, _c, _d, _e, _f
      if (svg) {
        ;(_a = pixelRatioSetting == null ? void 0 : pixelRatioSetting.settingEl) == null
          ? void 0
          : _a.hide()
        ;(_b = noFontExportSetting == null ? void 0 : noFontExportSetting.settingEl) == null
          ? void 0
          : _b.show()
        ;(_c =
          transparentBackgroundSetting == null ? void 0 : transparentBackgroundSetting.settingEl) ==
        null
          ? void 0
          : _c.hide()
      } else {
        ;(_d = pixelRatioSetting == null ? void 0 : pixelRatioSetting.settingEl) == null
          ? void 0
          : _d.show()
        ;(_e = noFontExportSetting == null ? void 0 : noFontExportSetting.settingEl) == null
          ? void 0
          : _e.hide()
        ;(_f =
          transparentBackgroundSetting == null ? void 0 : transparentBackgroundSetting.settingEl) ==
    ...
    let svg = false
    new import_obsidian18.Setting(modal.contentEl)
    ...
    pixelRatioSetting = new import_obsidian18.Setting(modal.contentEl)
      .setName("Pixel ratio")
    ...
    noFontExportSetting = new import_obsidian18.Setting(modal.contentEl)
      .setName("Skip font export")
      .setDesc(
        "This will not include the fonts in the exported SVG. This will make the SVG file smaller.",
      )
      .addToggle((toggle) =>
        toggle.setValue(noFontExport).onChange((value) => (noFontExport = value)),
      )
    ...
    new import_obsidian18.Setting(modal.contentEl)
      ...
      .addToggle((toggle) => toggle.setValue(watermark).onChange((value) => (watermark = value)))
    ...
    transparentBackgroundSetting = new import_obsidian18.Setting(modal.contentEl)
      .setName("Transparent background")
      .setDesc("This will make the background of the image transparent.")
      .addToggle((toggle) =>
        toggle.setValue(transparentBackground).onChange((value) => (transparentBackground = value)),
      )
    new import_obsidian18.Setting(modal.contentEl).addButton((button) =>
      button
        .setButtonText("Save")
        .setCta()
        .onClick(async () => {
  ...
  async exportImage(
    ...
    const edgesToExport = [...canvas.edges.values()].filter((edge) => {
      const edgeData = edge.getData()
      return (
        nodesToExportIds.includes(edgeData.fromNode) && nodesToExportIds.includes(edgeData.toNode)
      )
    })
    ...
    try {
      ...
      let canvasScale = parseFloat(
        ((_a = canvas.canvasEl.style.transform.match(/scale\((\d+(\.\d+)?)\)/)) == null
          ? void 0
          : _a[1]) || "1",
      )
      ...
      canvasScale = parseFloat(
        ((_b = canvas.canvasEl.style.transform.match(/scale\((\d+(\.\d+)?)\)/)) == null
          ? void 0
          : _b[1]) || "1",
      )
      ...
      if (actualAspectRatio > targetAspectRatio) width = height * targetAspectRatio
      ...
      if (unloadedNodes.length === 0) {
        ...
        const filter = (element) => {
          var _a2, _b2, _c2, _d
          if (
            ((_a2 = element.classList) == null ? void 0 : _a2.contains("canvas-node")) &&
            !nodeElements.includes(element)
          )
            return false
          if (
            ((_c2 = (_b2 = element.parentElement) == null ? void 0 : _b2.classList) == null
              ? void 0
...
var FloatingEdgeCanvasExtension = class extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:data-loaded:after", (canvas, data, setData) =>
        this.onLoadData(canvas, data),
      ),
    )
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-moved", (canvas, node) =>
        this.onNodeMoved(canvas, node),
      ),
    )
  ...
  getFloatingEdgeDropZoneForNode(node) {
    ...
    return {
      minX:
        ...
        (nodeElClientBoundingRect.width - nodeFloatingEdgeDropZoneSize.width) / 2,
      minY:
        nodeElClientBoundingRect.top +
        (nodeElClientBoundingRect.height - nodeFloatingEdgeDropZoneSize.height) / 2,
      maxX:
...
var NodeExposerExtension = class extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-changed", (_canvas, node) => {
        var _a, _b
        const nodeData = node == null ? void 0 : node.getData()
        if (!nodeData) return
        this.setDataAttributes(node.nodeEl, nodeData)
        const iframe =
          (_b = (_a = node.nodeEl.querySelector("iframe")) == null ? void 0 : _a.contentDocument) ==
          null
            ? void 0
            : _b.body
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on(
        ...
        (_canvas, node, editing) => {
          ...
          const iframe =
            (_b =
              (_a = node.nodeEl.querySelector("iframe")) == null ? void 0 : _a.contentDocument) ==
            null
              ? void 0
              : _b.body
          if (!iframe) return
...
var NodeInteractionExposerExtension = class extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:node-interaction", (canvas, node) => {
        const nodeData = node == null ? void 0 : node.getData()
        if (!nodeData) return
...
var EdgeExposerExtension = class extends CanvasExtension {
  ...
  init() {
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:edge-changed", (_canvas, edge) => {
        var _a, _b, _c, _d
        const edgeData = edge == null ? void 0 : edge.getData()
        if (!edgeData) return
        for (const exposedDataKey of getExposedEdgeData(this.plugin.settings)) {
          const datasetPairs =
            edgeData[exposedDataKey] && typeof edgeData[exposedDataKey] === "object"
...
var CanvasWrapperExposerExtension = class extends CanvasExtension {
  ...
  init() {
    ...
    this.plugin.registerEvent(
      this.plugin.app.workspace.on("advanced-canvas:dragging-state-changed", (canvas, dragging) => {
...
var AdvancedCanvasPlugin = class extends import_obsidian19.Plugin {
  ...
  createFileSnapshot(path, content) {
    ...
    const fileRecoveryPlugin =
      (_a = this.app.internalPlugins.plugins["file-recovery"]) == null ? void 0 : _a.instance
