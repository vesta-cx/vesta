...
ParsedListQuery          { where, orderBy, limit, offset }
  │
  ▼
Consumer composes:       and(authWhere, query.where)
...
packages/drizzle-query-factory/
  src/
    ...
    parse-list-query.ts   core factory function
...
- [types.ts](./src/types.md) — QueryInput, FilterOp, ColumnFilter, CustomFilter, ListQueryConfig, ParsedListQuery, response envelopes
...
const [rows, [{ total }]] = await Promise.all([
  ...
  db.select({ total: sql<number>`count(*)` }).from(resources).where(finalWhere),
]);
