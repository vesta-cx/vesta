...
var generateFontFace = (font) => {
  ...
  if (stretch) result += `${spacer}font-stretch: ${stretch.min}% ${stretch.max}%;`
...
var applyCss = async (id, plugin) => {
  ...
  const css = await plugin.app.vault.adapter.read(`${plugin.app.vault.configDir}/fonts/${id}.css`)
  if (existing) {
...
var getDOMException = (errorMessage) =>
  globalThis.DOMException === void 0 ? new AbortError(errorMessage) : new DOMException(errorMessage)
var getAbortedReason = (signal) => {
  const reason =
    signal.reason === void 0 ? getDOMException("This operation was aborted.") : signal.reason
      ...
      () => {
        ...
        if (message === false) {
          ...
          timeoutError.message =
            message != null ? message : `Promise timed out after ${milliseconds} milliseconds`
          reject(timeoutError)
        }
      },
      milliseconds,
        ...
        try {
          ...
          if (options.signal) {
            operation = Promise.race([
              operation,
              __privateMethod(this, _throwOnAbort, throwOnAbort_fn).call(this, options.signal),
            ])
          }
  ...
  if (__privateGet(this, _intervalId) === void 0) {
    ...
    if (delay < 0) {
      __privateSet(
        this,
        _intervalCount,
        __privateGet(this, _carryoverConcurrencyCount) ? __privateGet(this, _pending) : 0,
      )
    } else {
...
tryToStartAnother_fn = function () {
  ...
  if (!__privateGet(this, _isPaused)) {
    ...
    if (
      __privateGet(this, _doesIntervalAllowAnother, doesIntervalAllowAnother_get) &&
      __privateGet(this, _doesConcurrentAllowAnother, doesConcurrentAllowAnother_get)
    ) {
...
var getAxes = (variable) => {
  ...
  const isStandard = axes.every((axis) => ["wght", "wdth", "slnt", "opsz"].includes(axis))
  if (isStandard) {
    return "standard"
  }
  return "full"
...
var SearchModal = class extends import_obsidian2.SuggestModal {
  ...
  filterFonts(query) {
    return this.listCache.filter((font) => font.family.toLowerCase().includes(query.toLowerCase()))
  }
...
var SearchActiveModal = class extends import_obsidian2.SuggestModal {
  ...
  onChooseSuggestion(font, _evt) {
    ...
    switch (this.type) {
      case "interface":
        changed = this.addFontToSettings(this.plugin.settings.interfaceFonts, font)
        break
      case "text":
        changed = this.addFontToSettings(this.plugin.settings.textFonts, font)
        break
      case "monospace":
        changed = this.addFontToSettings(this.plugin.settings.monospaceFonts, font)
        break
    }
...
var FontsourceSettingsTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    addFontSetting("Interface font", "Set base font for all of Obsidian.", "interface")
    addFontSetting("Text font", "Set font for editing and reading views.", "text")
    addFontSetting(
      "Monospace font",
    ...
    const removeFromAppliedFonts = (id) => {
      ...
      this.plugin.settings.textFonts = removeFromSettings(this.plugin.settings.textFonts, id)
    ...
    const fonts = this.plugin.settings.fonts.sort((a, b) => a.family.localeCompare(b.family))
    for (const font of fonts) {
      ...
      new import_obsidian3.Setting(containerEl)
        ...
        .addButton((button) => {
          ...
          button.onClick(async () => {
            ...
            try {
              await vault.adapter.remove(`${vault.configDir}/fonts/${font.id}.css`)
              removeCss(font.id)
              this.plugin.settings.fonts = removeFromSettings(this.plugin.settings.fonts, font.id)
              removeFromAppliedFonts(font.id)
              this.plugin.saveSettings()
