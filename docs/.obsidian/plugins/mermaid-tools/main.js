...
var CategoryService = class {
  ...
  loadCategories(customCategories, defaultCategorySortOrders = {}) {
    const defaultCategories = DEFAULT_CATEGORIES.map((cat) => ({
      ...cat,
      sortOrder:
        defaultCategorySortOrders[cat.id] !== void 0
          ? defaultCategorySortOrders[cat.id]
          : cat.sortOrder,
    }))
...
var MermaidElementService = class {
  ...
  fixSortOrder(element, plugin) {
    const elementsFromSameCategory = plugin.settings.elements.filter(
      (el) => el.categoryId === element.categoryId,
    )
  ...
  getSampleDiagram(categoryId) {
    ...
    console.warn(
      `[Mermaid Tools] No sample diagram found for category: ${category.name}, using default sample`,
    )
    return this.wrapForPastingIntoEditor(this.wrapWithMermaidBlock("flowchart TD\nStart --> End"))
  }
...
\`\`\``
  ...
  wrapAsCompleteDiagram(element) {
    ...
    if (!validDiagramTypes.includes(firstWord)) {
      console.warn(
        `[Mermaid Tools] Potentially invalid diagram type "${firstWord}" in category ${element.categoryId}. This may cause rendering errors.`,
      )
    }
...
var EditMermaidElementModal = class extends import_obsidian2.Modal {
  ...
  async onOpen() {
    ...
    const elementDescriptionEl = elementDescriptionContainerEl.createEl("input", {
    ...
    const elementContentEl = elementContentContainerEl.createEl("textarea", {
    ...
    elementContentEl.onchange = async (e) => {
      ...
      const { svg: svg2 } = await this._mermaid.render(
        renderEl.id,
    ...
    const { svg } = await this._mermaid.render(
      renderEl.id,
      this._plugin._mermaidElementService.wrapAsCompleteDiagram(this._element),
    )
...
Start --> Stop`,
    ...
    const modal = new EditMermaidElementModal(
      plugin.app,
      plugin,
      mermaid,
      newElement,
...
function createIntegratedCategorySection(containerEl, plugin, categoryService, mermaid) {
  ...
  allCategories.forEach((category) => {
    ...
    const elementCount = plugin.settings.elements.filter(
    ...
    addElementButton.onmouseenter = () =>
      (addElementButton.style.backgroundColor = "var(--background-modifier-hover)")
...
Start --> Stop`,
    ...
    moveUpButton.onmouseenter = () =>
      (moveUpButton.style.backgroundColor = "var(--background-modifier-hover)")
    ...
    moveDownButton.onmouseenter = () =>
      (moveDownButton.style.backgroundColor = "var(--background-modifier-hover)")
    ...
    editButton.onmouseenter = () =>
      (editButton.style.backgroundColor = "var(--background-modifier-hover)")
    ...
    deleteButton.onclick = (e) => {
      e.stopPropagation()
      const elementsInCategory = plugin.settings.elements.filter(
        (el) => el.categoryId === category.id,
      )
...
function createTopRowBtns(parentEl, buttons) {
  buttons.forEach((btn) => {
    const b = new import_obsidian5.ButtonComponent(parentEl)
