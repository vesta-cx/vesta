...
function flatten(blockers, sync, async2, fn) {
  ...
  var restore = capture()
  function run3() {
    Promise.all(async2.map((expression) => async_derived(expression)))
      .then((result) => {
        restore()
        try {
  ...
  return new Proxy(
    ...
    value,
    {
      ...
      has(target, prop2) {
        ...
        if (
          s !== void 0 ||
          (active_effect !== null && (!has || get_descriptor(target, prop2)?.writable))
        ) {
          if (s === void 0) {
            s = with_parent(() => {
    ...
    var target =
      ...
      root6.host
        ? /** @type {ShadowRoot} */
          root6
        : /** @type {Document} */
          (root6.head ?? /** @type {Document} */ root6.ownerDocument.head)
...
function AddPropInput($$anchor, $$props) {
  ...
  let totalInputs = prop($$props, "totalInputs", 11, 0),
    typeVal = prop($$props, "typeVal", 11, "text"),
    nameVal = prop($$props, "nameVal", 15, ""),
    valueVal = prop($$props, "valueVal", 15, "")
...
function AddPropForm($$anchor, $$props) {
  ...
  onMount(() => {
    $$props.defaultProps.length > 0
      ? addInputs($$props.defaultProps)
  ...
  function removeInput(id) {
    return __awaiter(this, void 0, void 0, function* () {
      set(
        inputEls,
        get(inputEls).filter((input) => input.id !== id),
        true,
      )
      yield tick()
...
var root3 = from_html(
  `<div><form><p class="msg svelte-n0gky0"> </p> <p>The following props will be added:</p> <ul></ul> <p>Are you sure you wish to proceed?</p> <button class="mod-warning" type="submit">Confirm</button> <button type="button">Cancel</button></form></div>`,
)
var $$css3 = {
  hash: "svelte-n0gky0",
...
var PropModal = class extends import_obsidian2.Modal {
  ...
  onSubmit(props) {
    this.props = props
    new AddConfirmModal(this.app, this.props, this.alterProp, this.onConfirm.bind(this)).open()
  }
  onOpen() {
    this.titleEl.createEl("h2", { text: "Add Properties" })
...
var SettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl)
      ...
      .setDesc("Set delimiter to use when creating a list.  Commas(,) are used by default.")
      .addText((text2) => {
...
function RemovePropForm($$anchor, $$props) {
  ...
  user_effect(() => {
    ...
    set(
      inputs,
      sortedNames.map((name) => ({ name, isChecked: false })),
      true,
    )
    set(checkCount, 0)
  })
  let isMaxChecked = user_derived(
    () => get(inputs).length > 0 && get(checkCount) >= get(inputs).length,
  )
  ...
  function toggleAll() {
    ...
    set(
      inputs,
      get(inputs).map((input) =>
        Object.assign(Object.assign({}, input), { isChecked: shouldCheckAll }),
      ),
      true,
    )
  ...
  function onSubmit(e) {
    ...
    if (get(checkCount) === 0) {
      ...
      get(errorEl) === null || get(errorEl) === void 0
        ? void 0
        : get(errorEl).classList.remove("hidden")
      return
    }
    let propNames = get(inputs)
  ...
  each(
    ...
    ($$anchor2, input, $$index) => {
      ...
      bind_checked(
        input_1,
        () => get(input).isChecked,
        ($$value) => (get(input).isChecked = $$value),
      )
...
var root5 = from_html(
  `<div><form><p> </p> <ul></ul> <p>Are you sure you wish to proceed?</p> <button class="mod-warning" type="submit">Delete</button> <button type="button">Cancel</button></form></div>`,
)
...
var RemoveModal = class extends import_obsidian5.Modal {
  ...
  onSubmit(props) {
    this.props = props
    new RemoveConfirmModal(this.app, this.props, this.onConfirm.bind(this)).open()
  }
  onOpen() {
    this.titleEl.createEl("h2", { text: "Remove Properties" })
...
var MultiPropPlugin2 = class extends import_obsidian7.Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.workspace.on("tab-group-menu", (menu) => {
        const obj = this._getFilesFromTabGroup(this.app.workspace.getLeaf(false))
        menu.addItem((item) => {
          item
            .setIcon("archive")
    ...
    this.addCommand({
      ...
      callback: async () => {
        const files = this._getFilesFromTabGroup(this.app.workspace.getLeaf(false))
        if (!files || !files.length) {
          new import_obsidian7.Notice(
  ...
  async createPropModal(iterable) {
    ...
    iterateFunc = async (props) =>
      this.searchFolders(iterable, await this.addPropsCallback(props, allFiles.length))
    let defaultProps
