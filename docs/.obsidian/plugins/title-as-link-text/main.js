...
var LinkUpdater = class {
  ...
  async updateLinksInNote(file) {
    ...
    let newFileContent = fileContent.replace(markdownLinkRegex, (match, linkText, linkUrl) => {
        ...
        if (linkedFile) {
          ...
          if (linkedCache) {
            ...
            if (linkText) {
              ...
              const baseLinkName =
                ((_a = linkPath.split("/").pop()) == null ? void 0 : _a.replace(".md", "")) || ""
      ...
      for (const note of allNotes) {
        ...
        if (embedsAndLinks) {
          for (const link_data of embedsAndLinks) {
            const firstLinkPath = this.metadataCache.getFirstLinkpathDest(link_data.link, note.path)
  ...
  getPageTitle(cache, filePath) {
    const frontMatterTitle =
      this.settings.useFrontmatterTitle &&
      cache.frontmatter &&
      cache.frontmatter[this.settings.frontmatterTitleProperty]
    const firstHeading =
      this.settings.useFirstHeading &&
      cache.headings &&
      cache.headings.length > 0 &&
...
var TitleAsLinkTextPlugin = class extends import_obsidian.Plugin {
  async onload() {
    ...
    this.linkUpdater = new LinkUpdater(this.app.vault, this.app.metadataCache, this.settings)
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (activeFile && activeFile.extension === "md") {
          const backLinks = await this.linkUpdater.updateBackLinks(
            activeFile,
            activeFile.path,
            false,
          )
          if (backLinks) {
            new import_obsidian.Notice(
              `Updated the link text of ${backLinks} Markdown link(s) in the current file.`,
            )
          }
        } else {
...
var TitleAsLinkTextSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.useFirstHeading).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl)
      .setName("Use aliases")
      .setDesc(
        "Match link text against frontmatter aliases. When disabled, only the title will be used.",
      )
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.useAliases).onChange(async (value) => {
    ...
    new import_obsidian.Setting(containerEl)
      ...
      .setDesc(
        "Minimum similarity score (0.0 to 1.0) required for alias matching. Higher values require closer matches.",
      )
      .addText((text) =>
        text
          .setPlaceholder("0.65")
