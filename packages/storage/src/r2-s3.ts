...
import type { ListResult, ListedObject, StorageObject, StorageProvider } from './types.js';
...
export class R2S3StorageProvider implements StorageProvider {
	...
	constructor(config: R2S3Config) {
		const { accountId, accessKeyId, secretAccessKey, bucket, region = 'auto' } = config;
		this.bucket = bucket;
	...
	async list(
		...
		const objects: ListedObject[] = (response.Contents ?? []).map((o) => ({
			key: o.Key ?? '',
			size: o.Size ?? 0,
			uploaded: o.LastModified ? new Date(o.LastModified) : new Date(0)
		}));
