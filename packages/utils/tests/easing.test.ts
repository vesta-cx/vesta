...
describe("Easing Functions", () => {
			...
			allEasingFunctions.forEach(({ name, fn }) => {
				testValues.forEach((t) => {
					...
					if (overshootFunctions.includes(name)) {
						...
						expect(
							result,
							`${name}(${t}) should be reasonable (overshoot allowed)`,
						...
						expect(
							result,
							`${name}(${t}) should be between 0 and 1`,
	...
	describe("Quadratic", () => {
		...
		it("easeInOutQuad should be symmetric", () => {
			expect(easeInOutQuad(0.25)).toBeCloseTo(
				1 - easeInOutQuad(0.75),
				5,
			);
	...
	describe("Cubic", () => {
		...
		it("easeOutCubic should end slower than quadratic", () => {
			...
			expect(easeOutCubic(0.5)).toBeGreaterThan(
				easeOutQuad(0.5),
			);
		});

		it("easeInOutCubic should be symmetric", () => {
			expect(easeInOutCubic(0.25)).toBeCloseTo(
				1 - easeInOutCubic(0.75),
				5,
			);
	...
	describe("Quartic", () => {
		it("easeInQuartic should start slower than cubic", () => {
			...
			expect(easeInQuartic(0.5)).toBeLessThan(
				easeInCubic(0.5),
			);
		});
		...
		it("easeOutQuartic should end slower than cubic", () => {
			...
			expect(easeOutQuartic(0.5)).toBeGreaterThan(
				easeOutCubic(0.5),
			);
		});
	});
	...
	describe("Quintic", () => {
		it("easeInQuintic should start slower than quartic", () => {
			...
			expect(easeInQuintic(0.5)).toBeLessThan(
				easeInQuartic(0.5),
			);
		});
		...
		it("easeOutQuintic should end slower than quartic", () => {
			...
			expect(easeOutQuintic(0.5)).toBeGreaterThan(
				easeOutQuartic(0.5),
			);
		});
	});
	...
	describe("Sine", () => {
		...
		it("easeInOutSine should be symmetric", () => {
			expect(easeInOutSine(0.25)).toBeCloseTo(
				1 - easeInOutSine(0.75),
				5,
			);
	...
	describe("Circular", () => {
		...
		it("easeInOutCircular should be symmetric", () => {
			expect(easeInOutCircular(0.25)).toBeCloseTo(
				1 - easeInOutCircular(0.75),
				5,
			);
	...
	describe("Back (Overshoot)", () => {
		...
		it("easeInOutBack should be symmetric", () => {
			expect(easeInOutBack(0.25)).toBeCloseTo(
				1 - easeInOutBack(0.75),
				5,
			);
	...
	describe("Bounce", () => {
		...
		it("easeInBounce should be inverse of easeOutBounce", () => {
			expect(easeInBounce(0.3)).toBeCloseTo(
				1 - easeOutBounce(0.7),
				2,
			);
			expect(easeInBounce(0.5)).toBeCloseTo(
				1 - easeOutBounce(0.5),
				2,
			);
		});
		...
		it("ease-in functions should be monotonic increasing", () => {
			...
			easeInFunctions.forEach((fn) => {
				for (let i = 1; i < testPoints.length; i++) {
					...
					expect(
						curr,
						`Function should be monotonic increasing at ${testPoints[i]}`,
