...
const REVOKED_FILE = process.env['EUTERPE_REVOKED_FILE'] ?? '.euterpe-revoked-keys';
...
const main = (): void => {
	...
	const keysPath = path.isAbsolute(KEYS_FILE) ? KEYS_FILE : path.join(process.cwd(), KEYS_FILE);
	const revokedPath = path.isAbsolute(REVOKED_FILE) ? REVOKED_FILE : path.join(process.cwd(), REVOKED_FILE);
	...
	if (foundInFile) {
		fs.writeFileSync(keysPath, newLines.join('\n') + (newLines.length > 0 ? '\n' : ''), 'utf8');
	...
	console.log('Key added to', REVOKED_FILE, '(was not in keys file; likely env-only)');
};
