...
// Extract the derived API version environment (e.g., "v0", "v1")
// This is auto-generated by sync-api-version.mjs during `pnpm deploy`
const configRaw = await readFile(apiConfigPath, "utf8");
const envMatch = configRaw.match(/API_ENV = "([^"]+)"/);
if (!envMatch) {
	throw new Error(
		"Could not read API_ENV from src/config/api-version.ts",
	);
}
const apiEnv = envMatch[1];
