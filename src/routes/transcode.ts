...
transcodeRoutes.get('/status', async (c) => {
	...
	const [job] = await db.select().from(transcodeJobs).where(eq(transcodeJobs.id, jobId));
...
transcodeRoutes.post('/', async (c) => {
	...
	if (!config.filename || typeof config.uploadPrefix !== 'string') {
		return c.json({ error: 'Config must include filename and uploadPrefix' }, 400);
	}
	const storageConfig = config.storage;
	if (!storageConfig?.type) {
		return c.json({ error: 'Config must include storage (type, bucket, credentials)' }, 400);
	}
	if (
		...
		return c.json({ error: 'Storage config must include bucket, accessKeyId, secretAccessKey' }, 400);
	}
	...
	const tmpPath = path.join(tmpDir, `euterpe_${jobId}_${path.basename(file.name || 'audio.flac')}`);
	...
	void (async () => {
		...
		try {
			...
			const result = await transcode(tmpPath, config, storage);
