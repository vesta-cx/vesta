...
const CODECS: CodecDef[] = [
	{ id: 'aac', label: 'AAC fMP4', badge: 'control', src: '/aac/playlist.m3u8' },
	{ id: 'opus', label: 'Opus fMP4', badge: 'test', src: '/opus/playlist.m3u8' },
	{ id: 'flac', label: 'FLAC fMP4', badge: 'test', src: '/flac/playlist.m3u8' },
	{ id: 'mp3', label: 'MP3', badge: 'control', src: '/mp3/playlist.m3u8' }
];
...
const setStatus = (card: HTMLElement, msg: string, level: '' | 'success' | 'error' = '') => {
...
const handlePlay = (codec: CodecDef, card: HTMLElement) => {
	const w = window as Window & {
		cast?: { framework: { CastContext: { getInstance(): CastContextApi } } };
		chrome?: { cast?: { media?: { MediaInfo: new (u: string, t: string) => unknown; LoadRequest: new (m: unknown) => unknown } } };
	};
	...
	log(`Loading ${codec.label} via ${isNative ? 'native HLS' : 'HLS.js (MSE)'}`, 'info');
	...
	if (isNative) {
		...
		audio.addEventListener(
			'loadedmetadata',
			() => {
				log(`${codec.label}: loaded (native), duration=${audio.duration.toFixed(1)}s`, 'info');
				setStatus(card, `Playing (native HLS, ${audio.duration.toFixed(1)}s)`, 'success');
			},
			{ once: true }
		);
		...
		audio.addEventListener(
			...
			() => {
				...
				log(`${codec.label}: native error — ${msg}`, 'error');
		...
		audio.play().catch((e) => {
			log(`${codec.label}: play() rejected — ${e.message}`, 'error');
		...
		hls.on(Hls.Events.FRAG_LOADED, (_event, data) => {
			log(`${codec.label}: segment ${data.frag.sn} loaded`, 'info');
		});

		audio.addEventListener(
			'playing',
			() => {
				setStatus(card, `Playing (HLS.js, ${audio.duration.toFixed(1)}s)`, 'success');
			},
			{ once: true }
		);
		...
		log(`${codec.label}: no HLS support (no native, no MSE)`, 'error');
...
const handleAirPlay = (card: HTMLElement) => {
	const audio = card.querySelector('audio') as HTMLMediaElement & { webkitShowPlaybackTargetPicker?: () => void };
...
const hasAirPlayAPI = (el: HTMLMediaElement): boolean =>
	...
	typeof (el as HTMLMediaElement & { webkitShowPlaybackTargetPicker?: unknown }).webkitShowPlaybackTargetPicker === 'function';
...
const initCast = () => {
	...
	if (!appId) {
		log('Cast: chrome.cast.media missing — use Google Chrome for Cast support', 'warn');
	...
	try {
		...
		document.querySelectorAll('.cast-btn').forEach((btn) => ((btn as HTMLButtonElement).disabled = false));
	} catch (e) {
...
interface CastContextApi {
	...
	getCurrentSession(): { loadMedia(req: unknown): Promise<unknown> } | null;
}
...
const handleChromecast = async (codec: CodecDef, card: HTMLElement) => {
	...
	if (!w.chrome?.cast?.media?.MediaInfo || !w.chrome?.cast?.media?.LoadRequest) {
		log('Cast media API missing — use Google Chrome for full Cast support', 'error');
		setStatus(card, 'Use Google Chrome for Cast', 'error');
		return;
	}
	const baseEl = document.getElementById('cast-base-url') as HTMLInputElement | null;
	const formatEl = document.getElementById('cast-format') as HTMLSelectElement | null;
	...
	if (useHls) {
		...
		const flacContainerEl = document.getElementById('flac-container') as HTMLSelectElement | null;
		if (codec.id === 'mp3') {
			castPath = '/mp3/full.mp3';
			contentType = 'audio/mpeg';
			formatLabel = 'full.mp3';
		} else if (codec.id === 'flac' && flacContainerEl?.value === 'flac') {
			...
			formatLabel = codec.id === 'flac' ? 'full.mp4 (FLAC-in-MP4)' : 'full.mp4';
		}
	}
	const url = `${base.replace(/\/$/, '')}${castPath}`;
	if (base.includes('localhost') || base.includes('127.0.0.1')) {
		log('Cast base is localhost — Chromecast cannot reach it. Use your LAN IP or deployed URL', 'warn');
		setStatus(card, 'Chromecast cannot reach localhost — set Cast base URL', 'error');
		return;
	}
	const ctx = w.cast.framework.CastContext.getInstance();
	let session = ctx.getCurrentSession();
	if (!session) {
		log(`Requesting Cast session to cast ${codec.label}...`, 'info');
	...
	try {
		...
		log(`${codec.label}: casting via ${formatLabel} → ${castPath}`, 'info');
		setStatus(card, `Casting ${codec.label} (${formatLabel})`, 'success');

		const media = (session as { getMediaSession?: () => { getEstimatedTime: () => number; media?: { duration?: number }; addUpdateListener: (fn: (alive: boolean) => void) => void; removeUpdateListener: (fn: (alive: boolean) => void) => void; seek: (req: unknown, onOk: () => void, onErr: (e: Error) => void) => void } }).getMediaSession?.();
		if (media) {
...
const startCastProgressSync = (
	card: HTMLElement,
	media: { getEstimatedTime: () => number; media?: { duration?: number }; addUpdateListener: (fn: (alive: boolean) => void) => void; removeUpdateListener: (fn: (alive: boolean) => void) => void; seek: (req: unknown, onOk: () => void, onErr: (e: Error) => void) => void }
) => {
	stopCastProgressSync();
	...
	const range = progressWrap.querySelector('input[type="range"]') as HTMLInputElement;
...
const stopCastProgressSync = () => {
	...
	document.querySelectorAll('.cast-progress').forEach((el) => el.remove());
};
...
const buildCards = () => {
	...
	for (const codec of CODECS) {
		...
		const airplayBtn = card.querySelector('[data-action="airplay"]') as HTMLButtonElement;

		if (hasAirPlayAPI(audio)) {
			const onAvailability = (e: Event) => {
				const ev = e as WebKitPlaybackTargetAvailabilityEvent;
				airplayBtn.disabled = ev.availability !== 'available';
			};
		...
		const castBtn = card.querySelector('[data-action="cast"]') as HTMLButtonElement;
		castBtn.addEventListener('click', () => handleChromecast(codec, card));

		card.querySelector('[data-action="play"]')!.addEventListener('click', () => handlePlay(codec, card));
		card.querySelector('[data-action="stop"]')!.addEventListener('click', () => handleStop(card));
...
const onCastReady = (available: boolean) => {
	initCast();
	if (!available) log('Cast: API reported no devices — button enabled anyway, click to try', 'info');

	const w = window as Window & { cast?: { framework: { CastContext: { getInstance(): { addEventListener: (t: string, fn: (e: { sessionState: string }) => void) => void } } } } };
	w.cast?.framework?.CastContext?.getInstance?.()?.addEventListener?.(
		'SESSION_STATE_CHANGED',
		(e: { sessionState: string }) => {
			if (e.sessionState === 'SESSION_ENDED') stopCastProgressSync();
		}
	);
};
window.addEventListener('castready', ((e: CustomEvent<{ available: boolean }>) => onCastReady(e.detail.available)) as EventListener);
if ((window as Window & { __castApiAvailable?: boolean }).__castApiAvailable !== undefined) onCastReady(true);
...
const castBaseInput = document.getElementById('cast-base-url') as HTMLInputElement | null;
if (castBaseInput) {
	castBaseInput.placeholder = window.location.origin;
	if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
		castBaseInput.placeholder = 'https://YOUR_LAN_IP:5173';
		log('On localhost — for Cast, open via LAN IP or paste it in Cast base URL', 'info');
	}
}
